...
- ~~Rate-limit / block abusive IPs at Caddy (quickest stopgap).~~ **Done** — see
  ...
  nothing of it is built yet, and [`crowdsec-plan.md`](crowdsec-plan.md) is the
  plan. The tiered limits stay as they are regardless: CrowdSec cannot replace
  the aggregate zones, for the reason the 2026-08-04 section gives.
